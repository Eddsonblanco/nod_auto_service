...
import { Box, Typography, InputBase, Select, MenuItem, Input, IconButton } from '@material-ui/core'
...
const Pagination = props => {
  ...
  const _handleChange = ev => {
    ...
    if(/^[0-9]*$/g.test(value))
      setCurrentPage(prevState => value > total ? prevState : value)
  ...
  return (
    <Box className={classes.boxStyle}>
      ...
      <Box className={classes.boxStyle}>
        ...
        <Typography>{total > 0 ? total : 1}</Typography>
        <IconButton
          className={classes.rootRightIcon}
          disabled={parseInt(currentPage) === parseInt(total)}
